...
set_partitions() {
    ...
    if grep -e CONFIG_MICROPY_USE_ARDUINO_LAYOUT=y sdkconfig > /dev/null; then
        ...
        echo "# -------------------------------------------------------" >> partitions_mpy.csv
        echo "nvs,      data, nvs,     0x9000,  0x5000," >> partitions_mpy.csv
        echo "otadata,  data, ota,     0xe000,  0x2000," >> partitions_mpy.csv
        echo "app0,     app,  ota_0,   0x10000, 0x140000," >> partitions_mpy.csv
        echo "app1,     app,  ota_1,   0x650000,0x640000," >> partitions_mpy.csv
        echo "eeprom,   data, 0x99,    0xc90000,0x1000," >> partitions_mpy.csv
        echo "spiffs,   data, spiffs,  0xc91000,0x36F000," >> partitions_mpy.csv
        fs_start=$((0xc91000))
        fs_size=$((0x36F000))
    elif [ "${USE_OTA_PART}" == "CONFIG_MICROPY_USE_OTA=y" ]; then
